...
import type { AssetManifestV1, RightsManifestV1 } from '@roost2d/contracts';
import {
  buildAtlases, createInventory, createProject, findDuplicateImages, inspectImages,
  validateAnimationFile, validateAtlasFiles, validateRigFile, verifyManifestFiles,
  verifyRightsFiles, verifyToolingBoundary, verifyToolingPackageGraph, type AtlasBuildConfig
} from './index.js';

const [area, action, ...args] = process.argv.slice(2);

if (area === 'assets' && action === 'scan') {
  ...
  const manifest = JSON.parse(await readFile(resolve(manifestFile), 'utf8')) as AssetManifestV1; fail(await verifyManifestFiles(manifest, resolve(assetRoot)));
} else if (area === 'assets' && action === 'manifest') {
  const [directory, output = 'asset-inventory.json'] = args; requireValue(directory, 'roost2d assets manifest <directory> [output]'); await writeJson(output, await createInventory(resolve(directory)));
} else if (area === 'atlas' && action === 'build') {
  const [configFile] = args; requireValue(configFile, 'roost2d atlas build <config.json> [--dry-run]'); const config = JSON.parse(await readFile(resolve(configFile), 'utf8')) as AtlasBuildConfig; console.log(JSON.stringify(await buildAtlases(config, { dryRun: args.includes('--dry-run') }), null, 2));
  ...
  const [rightsFile, sourceRoot] = args; requireValue(rightsFile && sourceRoot, 'roost2d licenses validate <rights.json> <source-root>'); const manifest = JSON.parse(await readFile(resolve(rightsFile), 'utf8')) as RightsManifestV1; fail(await verifyRightsFiles(manifest, resolve(sourceRoot)));
  ...
  throw new Error('commands: assets scan|validate|manifest, atlas build|validate, rig validate, animation validate, licenses validate, project create');
}

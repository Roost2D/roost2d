...
`0.7.0` is the current stable npm-channel release. The major version remains zero while the
...
- public exports captured in `reports/public-api.json` are preserved throughout the `0.7.x` line unless a
  security or correctness defect makes that impossible;
- removals require a documented deprecation path and a release note;
- asset manifest, rig, animation, and rights schemas remain backward-compatible throughout the `0.7.x` line;
